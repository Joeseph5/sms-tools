...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    temp1 = fs / f1
    temp2 = fs / f2
    M = int(temp1 * temp2 / gcd(temp1,temp2))
    ...
    # less than -240dB equals to 0
    
    return mX

#W = 300
#signal = np.cos(2 * np.pi * 80 / 10000 * np.arange(W)) + np.cos(2 * np.pi * 200 / 10000 * np.arange(W))
#print(minimizeEnergySpreadDFT(signal, 10000, 80, 200))
